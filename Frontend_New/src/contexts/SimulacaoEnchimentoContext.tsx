...
export interface EnchimentoSimValues {
...
function computeValues(fase: Fase, p: number): EnchimentoSimValues {
  let pistao = 0;
...
interface SimCtx {
  ...
  values: EnchimentoSimValues;
}

const SimulacaoEnchimentoContext = createContext<SimCtx>({
  simulacaoAtiva: true,
  values: DEFAULT_VALUES,
});

export const SimulacaoEnchimentoProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [values, setValues] = useState<EnchimentoSimValues>(DEFAULT_VALUES);

  // 🐢 setInterval em vez de requestAnimationFrame: a página de Enchimento é
  ...
  return (
    <SimulacaoEnchimentoContext.Provider value={{ simulacaoAtiva: true, values }}>
      {children}
    </SimulacaoEnchimentoContext.Provider>
  );
};

export const useSimulacaoEnchimento = () => useContext(SimulacaoEnchimentoContext);
