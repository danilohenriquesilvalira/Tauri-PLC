...
interface MotorEnchimentoProps {
  websocketValue: number; // Valor do inteiro (índice 8 para esquerdo, 9 para direito)
  side?: 'direito' | 'esquerdo';
  editMode?: boolean;
}

const MotorEnchimento: React.FC<MotorEnchimentoProps> = ({ 
  websocketValue = 0,
  side = 'direito',
  editMode = false
}) => {
  
  // Define colors based on websocket integer value (0, 1, 2)
  const getMainColor = () => {
    switch (websocketValue) {
      case 1: return "#2ecc71"; // Verde para operacional
      case 2: return "#e74c3c"; // Vermelho para falha
      default: return "#95a5a6"; // Cinza para inativo (0 ou outros valores)
    }
  };

  // Cor principal do motor baseada no valor websocket
  ...
  const getSecondaryColor = () => {
    switch (websocketValue) {
      ...
      case 2: return "#c0392b"; // Vermelho escuro para falha
      default: return "#7f8c8d"; // Cinza escuro para inativo
    }
  };
...
export default MotorEnchimento;

// Estados do motor baseado no valor websocket:
// websocketValue = 0 -> Cinza (inativo)
// websocketValue = 1 -> Verde (operando)
// websocketValue = 2 -> Vermelho (falha)
